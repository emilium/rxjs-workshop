...
class Effects {
  init() {
    ...
    search.input$
      .pipe(
        ...
        switchMap((query: string) =>
          loadUsers(new URLSearchParams({ username_like: query }))
        )
      )
      .subscribe((users) => {
    ...
    const selectedUsersPosts$ = selectedCmdPressUsers$
      .pipe
      //...
      ();
    selectedUsersPosts$.subscribe((postsByUser: any) => {
      const posts = postsByUser.reduce(
        (a: IPost[], c: IPost[]) => [...a, ...c],
        []
      );
      userPostList.hideLoading();
